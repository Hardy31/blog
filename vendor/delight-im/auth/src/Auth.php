...
final class Auth extends UserManager {
	 ...
	 */
	public function register($email, $password, $username = null, $newName,callable $callback = null) {
		...
		$newUserId = $this->createUserInternal(false, $email, $password, $username, $newName, $callback);
	...
	public function confirmEmailAndSignIn($selector, $token, $rememberDuration = null) {
		...
		if (!$this->isLoggedIn()) {
			if ($emailBeforeAndAfter[1] !== null) {
				...
				$this->onLoginSuccessful($userData['id'], $userData['email'], $userData['username'], $userData['status'], $userData['roles_mask'], $userData['force_logout'], $userData['image']);
	...
	private function authenticateUserInternal($password, $email = null, $username = null, $rememberDuration = null, callable $onBeforeSuccess = null) {
		...
		$columnsToFetch = [ 'id', 'email', 'password', 'verified', 'username', 'status', 'roles_mask', 'force_logout', 'image'];
